...
public class ApiExceptionHandler {
    ...
    public ResponseEntity<Map<String, String>> handleNaoUnico(NonUniqueResultException ex) {
        return ResponseEntity.status(HttpStatus.CONFLICT).body(Map.of("erro", "Existem registros duplicados para este identificador."));
    }

    @ExceptionHandler(DataIntegrityViolationException.class)
    public ResponseEntity<Map<String, String>> handleIntegridade(DataIntegrityViolationException ex) {
        return ResponseEntity.status(HttpStatus.CONFLICT).body(Map.of("erro", "Dados duplicados: CRM, CPF, e-mail ou especialidade já cadastrado."));
    }
}
